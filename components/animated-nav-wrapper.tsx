...
import { getWordPressPages, organizePagesByParent, getSiteOptions, getWordPressEvents } from "@/lib/wordpress-api"

export async function AnimatedNavWrapper() {
  ...
  while (items.length < 6) {
    ...
    if (event) {
      const featuredImageUrl =
        event._embedded?.["wp:featuredmedia"]?.[0]?.source_url || event.acf?.background?.url || "/placeholder.svg"
      ...
      <AnimatedNav
        logo={
          <div className="relative h-8 w-auto aspect-square">
            {options?.logo_du_site?.url ? (
              <Image
                src={options.logo_du_site.url || "/placeholder.svg"}
